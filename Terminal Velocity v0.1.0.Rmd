...
```{r setWorkingDirectory, eval=FALSE, include=FALSE}
...
```{r getHigherTaxa, eval=FALSE, include=FALSE}
library("taxonlookup")
taxon <- lookup_table(as.character(tree$tip.label), by_species=TRUE)
df <-data.frame(species=TV$scrubbed_species_binomial, taxon, val=as.numeric(TV$log.velocity))
df -> TV
TV
head(tree$tip.label)
```
...
p2 <- facet_plot(p, panel="Dot", data=TV, geom=geom_point, aes(x=val, color=genus, pch=group))
p3 <- facet_plot(p2, panel='Bar', data=TV, geom=geom_segment, aes(x=0, xend=val, y=y, yend=y, size=2, color=order)) + theme_tree2()
facet_plot(p3, panel="Boxplot", data=TV, geom_boxplot, 
...
BD<-t(sapply(fit.bd,bd)) #fit.bd <- obj
par(mfrow=c(2,1))
...
```{r eval=FALSE, include=FALSE}
#scale branch lengths to deal with negative lengths (Error in check.tree(tree) : Negative branch lengths in tree)

#phylocomr::ph_bladj(, tree)
mytree$edge.length <- mytree$edge.length * 10^10
mytree <- force.ultrametric(mytree)
write.tree(mytree, "TV_TREE.newick")
```

```{r }
mytree <- tree
pbModel<-make.yule(mytree,sampling.f=(736/250000))
...
```{r }
bdModel<-make.bd(mytree,sampling.f=736/250000)
...
Assume standard deviation of 1/50 for all species – this comes from nowhere in particular, and is probably over-confident in the TV estimates for most species.
```{r }
trait.sd <- 1/20
```
...
he /sampling fraction is fglobalSamplingFraction is 736/391000 or `r $736/391000` 
, which refers to the 736 (`r len(trait)`) species in the dataset out of 391000 vascular plants (State of the World's Plants, 2016)
```{r }
...
```{r }
f.c <- make.stomata(constant.x, constant.x)
...
```{bash BAMMterminal}
cd ~/Documents/GitHub/PhyloTV
bamm -c template_trait.txt
```

```{bash BAMMterminal, eval=FALSE, include=FALSE}
cd ~/Documents/GitHub/PhyloTV
...
#eventfile
eventdata <- getEventData(NWK_BAMM_Tree, eventdata = "event_data_template_trait.txt", 
